...
<body id="wrap">
	...
	<div class="sjsc-title2">
		...
		<a href="index.html" class="sjsc-t2r"><img
			src="images/back.png" alt=""
			style="width:20px;height: 20px;padding-top: 
11px;padding-left: 5px"></a>
	</div>
		...
		<c:forEach items="${goods}" var="list" varStatus="s">
			<li>
				...
				<div class="gwc-md f-l">
					...
					<p class="gwc-p1" style="line-height: 30px;margin-top:-9px">
						<span style="font-size: 9px;">${list.goods_spe}200g份</span>
					</p>
					<p class="gwc-p1">
						<span style="color: #f60">￥${list.goods_price}</span>
					</p>
				</div> <%-- <c:if test="${list.goods_price!=0}"> --%> <a
				href="javascript:;"
				onclick="plus('${list.goods_id}','${list.goods_price}','${s.count}')"
				class="gwc-del f-r"><img src="images/11.png"
					style="width: 25px;height: 25px"></a> <a href="#"
				class="gwc-del f-r" id="goods_num${s.count}"
				style="padding-top:4px;width: 16px"> ${list.goods_num}</a> <a
				href="javascript:;"
				onclick="min('${list.goods_id}','${list.goods_price}','${s.count}')"
				class="gwc-del f-r"><img src="images/22.png"
					style="width: 25px;height: 25px"></a> <%-- </c:if>
	...
	<input type="hidden" value="${tprice}" id="tprice1" name="goods_total_menoy">
	<div class="gwc-ft">
		<p id="tnum">
			共${tnum}件商品，总计：<span style="color: #f60">￥${tprice }</span>
		</p>		
		<c:if test="${tnum==0}">
			<button onclick="showTip('请添加商品到购物车！')">结算</button>
		</c:if>
		<c:if test="${tnum!=0}">
			<button onclick="add()">结算</button>
		</c:if>
		<div style="clear:both;"></div>
	</div>
	...
	<script type="text/javascript">
		...
		function plus(goods_id, goods_price, sort) {
			...
			var tprice = (parseFloat(tprice1) + parseFloat(goods_price))
					.toFixed(1);
			$
					.ajax({
						url : 'cartUpdate.html',
						type : 'post',
						data : 'goods_id=' + goods_id + '&goods_price='
								+ goods_price + '&goods_num=' + goods_num
								+ '&s=1',
						success : function(rs) {
							var data = eval('(' + rs + ')');
							if (data.rs_code == 1) {
