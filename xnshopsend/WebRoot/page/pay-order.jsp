			...
			<c:forEach items="${map['list']}" var="list" varStatus="s">
				...
				<div class="my-k1">
					<ul class="my-p1">
						...
						<li class="my-spl f-l" id="orderId">订单号：${list.order_id}</li>
					...
					<c:forEach items="${map[ord]}" var="ordList">
						<dl class="my-dl1">
							...
							<dd>
								...
								<p class="my-dp1">
									价格：<span>￥${ordList.goods_price}</span>
								</p>
								<div class="my-jdt">
		...
		<c:forEach items="${map['list']}" var="list">
			<c:if test="${list.goods_total==0}">
				<button class="drdd-btn"
					onclick="window.location.href='orderUpdate.html?order_id=${list.order_id}'">确认下单</button>
			</c:if>
			<c:if test="${list.goods_total>0}">
